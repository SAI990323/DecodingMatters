...
do
    ...
    do
        echo $i
        CUDA_VISIBLE_DEVICES=$i python ./code/evaluate.py --base_model ./output_dir/${category}/ --train_file ${train_file} --info_file ${info_file} --category ${category} --test_data_path ./temp/${category}_base/${i}.csv --result_json_data ./temp/${category}_base/${i}.json &
    done
    wait
